(function(){
  ...
  function normalizePin(p,i){const t=String(p?.type||p?.kind||p?.category||p?.labelType||'other').toLowerCase();const name=String(p?.name||p?.label||p?.title||('Location '+(i+1)));const base={id:String(p?.id||uid('loc')),name,type:t,lat:Number.isFinite(Number(p?.lat))?Number(p.lat):null,lng:Number.isFinite(Number(p?.lng))?Number(p.lng):null,x:Number.isFinite(Number(p?.x))?Number(p.x):null,y:Number.isFinite(Number(p?.y))?Number(p.y):null,notes:String(p?.notes||p?.description||''),createdAt:p?.createdAt||now(),updatedAt:now()};if(t.includes('stand'))return base;if(t.includes('camera'))return base;if(t.includes('bedding'))return base;if(t.includes('food'))return base;if(t.includes('water'))return base;if(t.includes('scrape'))return base;if(t.includes('rub'))return base;return base;}
  ...
  window.BackwoodsData={version:VERSION,key:KEY,get,set,add,update,remove,export:exportData,import:importData,migrate};
  migrate();
})();
