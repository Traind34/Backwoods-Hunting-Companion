(function(){
  ...
  function activate(){
    localStorage.setItem(HYBRID_KEY,'1');setStateBase('hybrid');
  ...
  function deactivate(){localStorage.setItem(HYBRID_KEY,'0');setStateBase('road');const road=document.querySelector('#bwRoad');if(road){road.checked=true;road.dispatchEvent(new Event('change',{bubbles:true}))}const v=document.querySelector('#bwMapViewport');if(v)v.classList.remove('bw-hybrid-active')}
  function updateControls(){
    ...
    if(!host){
      ...
      const rl=road.closest('label');if(rl&&rl.parentElement)rl.parentElement.insertBefore(host,rl.nextSibling);
      host.querySelector('input').addEventListener('change',e=>{if(e.target.checked)activate()});
    }
    const cb=document.querySelector('#bwHybrid');if(cb)cb.checked=localStorage.getItem(HYBRID_KEY)==='1';
    if(localStorage.getItem(HYBRID_KEY)==='1')renderHybridTiles();
  }
