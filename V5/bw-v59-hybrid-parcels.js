(function(){
  ...
  function setStateBase(base){
    const s=readState();s.base=base;localStorage.setItem(KEY,JSON.stringify(s));
  }
  function css(){
  ...
  function ensureCanvas(){
    ...
    if(!canvas){
      canvas=document.createElement('canvas');canvas.id='bwHybridCanvas';canvas.setAttribute('aria-hidden','true');v.insertBefore(canvas,document.querySelector('#bwMapTiles'));
    }
    const r=v.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2);
  ...
  function tileUrl(x,y,z){
    const n=Math.pow(2,z);if(y<0||y>=n)return null;const xx=((x%n)+n)%n;
  ...
  function image(url){
    ...
    const p=new Promise(resolve=>{im.onload=()=>resolve(im);im.onerror=()=>resolve(null)});
    cache.set(url,p);return p;
  }
  function contourLayers(z){
    if(z<=8)return 'show:0';
    if(z<=11)return 'show:9';
    if(z===12)return 'show:14';
    return 'show:19';
  }
  function contourUrl(center,z,w,h){
    const world=256*Math.pow(2,z),cx=(center.lng+180)/360*world;
    const sin=Math.sin(center.lat*Math.PI/180),cy=(0.5-Math.log((1+sin)/(1-sin))/(4*Math.PI))*world;
    const west=lonFromPx(cx-w/2,z),east=lonFromPx(cx+w/2,z),north=latFromPx(cy-h/2,z),south=latFromPx(cy+h/2,z);
    const bbox=[mercX(west),mercY(south),mercX(east),mercY(north)].join(',');
    return 'https://cartowfs.nationalmap.gov/arcgis/rest/services/contours/MapServer/export'
      +'?bbox='+encodeURIComponent(bbox)+'&bboxSR=3857&imageSR=3857'
      +'&size='+Math.min(2048,w)+','+Math.min(2048,h)
      +'&format=png32&transparent=true&layers='+encodeURIComponent(contourLayers(z))
      +'&dpi=96&f=image';
  }
  async function renderHybrid(){
    if(!isHybrid())return;
    const v=document.querySelector('#bwMapViewport');const box=ensureCanvas();if(!v||!box||!ctx)return;
    const s=readState(),z=Math.max(3,Math.min(19,Number(s.zoom)||7)),center=s.center||{lat:41.2033,lng:-77.1945};
    const r=v.getBoundingClientRect(),w=box.w,h=box.h;
    ctx.clearRect(0,0,w,h);ctx.fillStyle='#53604f';ctx.fillRect(0,0,w,h);
    const world=256*Math.pow(2,z),cx=(center.lng+180)/360*world,cy=(0.5-Math.log((1+Math.sin(center.lat*Math.PI/180))/(1-Math.sin(center.lat*Math.PI/180)))/(4*Math.PI))*world;
    const urls=[];
    for(let tx=Math.floor(cx-w/512)-1;tx<=Math.ceil(cx+w/512)+1;tx++){
      for(let ty=Math.floor(cy-h/512)-1;ty<=Math.ceil(cy+h/512)+1;ty++){
        const url=tileUrl(tx,ty,z);if(!url)continue;
        urls.push({url,x:(tx-cx)*256+w/2,y:(ty-cy)*256+h/2});
      }
    }
    const imgs=await Promise.all(urls.map(t=>image(t.url).then(im=>({t,im}))));
    if(!isHybrid())return;
    imgs.forEach(({t,im})=>{if(im)ctx.drawImage(im,t.x,t.y,256,256)});
    const c=await image(contourUrl(center,z,w,h));
    if(c){ctx.save();ctx.globalAlpha=.78;ctx.drawImage(c,0,0,w,h);ctx.restore()}
  }
  function updateControls(){
    const sat=document.querySelector('#bwSatellite'),topo=document.querySelector('#bwTopo'),road=document.querySelector('#bwRoad');
    if(!sat||!topo||!road)return;
    ...
    if(!host){
      ...
      const roadLabel=road.closest('label');
      if(roadLabel&&roadLabel.parentElement)roadLabel.parentElement.insertBefore(host,roadLabel.nextSibling);
      ...
      host.querySelector('input').addEventListener('change',()=>{
        ...
        setHybrid(true);setStateBase('satellite');
    ...
    if(road.checked&&!isHybrid()){
      const v=document.querySelector('#bwMapViewport');if(v)v.classList.remove('bw-hybrid-active');
    }
  }
  function bindRoad(){
    const road=document.querySelector('#bwRoad');if(!road||road.dataset.v59)return;road.dataset.v59='1';
    road.addEventListener('change',()=>{setHybrid(false);const v=document.querySelector('#bwMapViewport');if(v)v.classList.remove('bw-hybrid-active');const cb=document.querySelector('#bwHybrid');if(cb)cb.checked=false});
  }
  function tick(){
    updateControls();bindRoad();
    const v=document.querySelector('#bwMapViewport');if(!v)return;
    if(isHybrid())v.classList.add('bw-hybrid-active');
