...
function moveHuntToday(){const v=document.querySelector(VIEW);if(!v)return;const els=[...v.querySelectorAll('button')].filter(b=>/hunt\s*today/i.test((b.textContent||'').trim()));els.forEach(el=>{el.style.position='absolute';el.style.right='10px';el.style.top='390px';el.style.bottom='auto';el.style.zIndex='55';el.style.transform='none'})}
